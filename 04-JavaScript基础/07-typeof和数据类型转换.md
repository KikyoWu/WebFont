- [07-typeof和数据类型转换](#07-typeof和数据类型转换)
  - [前言](#前言)
  ...
  - [其他的简单类型 --> String](#其他的简单类型----string)
    - [方法一（隐式类型转换）：字符串拼接](#方法一隐式类型转换字符串拼接)
    - [方法二：调用 toString()方法](#方法二调用-tostring方法)
    - [方法三（强制转换）：使用 String()函数](#方法三强制转换使用-string函数)
    - [prompt()：用户的输入](#prompt用户的输入)
  - [其他的数据类型 --> Number 【重要】](#其他的数据类型----number-重要)
    - [使用 Number() 函数](#使用-number-函数)
    - [使用 parseInt()函数：字符串 -> 整数](#使用-parseint函数字符串---整数)
    - [parseFloat()函数：字符串 --> 浮点数（小数）](#parsefloat函数字符串----浮点数小数)
  - [转换为 Boolean](#转换为-boolean)
  - [知识补充：其他进制的数字](#知识补充其他进制的数字)
  - [隐式类型转换](#隐式类型转换-1)
    - [isNaN() 函数](#isnan-函数)
    - [自增/自减运算符：`++`、`—-`](#自增自减运算符-)
    - [正号/负号：`+a`、`-a`](#正号负号a-a)
...
```
const a = 1;console.log(a == true); // 打印结果：trueconsole.log(typeof a); // 打印结果：number。可见，上面一行代码里，a 做了隐式类型转换，但是 a 的数据类型并没有发生变化，仍然是 Number 类型console.log(0 == true); // 打印结果：false
```
...
```
const result1 = '';const result2 = {a:'data1', b: 'data2'};if (result1) {    console.log('因为 result1的内容为空，所以代码进不了这里');}if (result2 && result2.a) {    // 接口返回了 result2，且 result2.a 里面有值，前端才做进一步的事情    console.log('代码能进来，前端继续在这里干活儿');}
```
